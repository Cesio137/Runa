...
namespace Nanometro {
    ...
    bool TCPClientSsl::send(const std::string &message) {
        if (!pool && !isConnected() && !message.empty())
            return false;
    ...
    bool TCPClientSsl::sendRaw(const std::vector<std::byte> &buffer) {
        if (!pool && !isConnected() && !buffer.empty())
            return false;;
    ...
    void TCPClientSsl::run_context_thread() {
        mutexIO.lock();
        while (tcp.attemps_fail <= maxAttemp) {
            if (onConnectionRetry && tcp.attemps_fail > 0)
                onConnectionRetry(tcp.attemps_fail);
    ...
    void TCPClientSsl::resolve(const std::error_code &error, const asio::ip::tcp::resolver::results_type &endpoints) {
        if (error) {
            tcp.error_code = error;
            if (onError)
                onError(tcp.error_code.value(), tcp.error_code.message());
    ...
    void TCPClientSsl::ssl_handshake(const std::error_code &error) {
        if (error) {
            tcp.error_code = error;
            if (onError)
                onError(tcp.error_code.value(), tcp.error_code.message());
    ...
    void TCPClientSsl::write(const std::error_code &error, const size_t bytes_sent) {
        if (error) {
            if (onError)
                onError(error.value(), error.message());
            return;
    ...
    void TCPClientSsl::read(const std::error_code &error, const size_t bytes_recvd) {
    }
} // Nanometro
